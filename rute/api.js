...
var creatorList = ['@JunedGntng','@alpha'];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'---------JunedGanteng'+'JUNED--GANS';
...
router.get('/find', async (req, res, next) => {
    ...
    if (apikey != 'NddBot') return res.json(loghandler.invalidKey)
...
router.get('/tiktod', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'NddBot') return res.json(loghandler.invalidKey)
